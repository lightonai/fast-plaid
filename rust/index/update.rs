...
/// Updates a loaded index in-memory with new document embeddings.
///
/// This function takes an already loaded index and updates it with new documents
/// by modifying the existing LoadedIndex structure directly. This is more efficient
/// than reloading the entire index from disk.
///
/// # Arguments
///
...
pub fn update_loaded_index(
    ...
    // Process new chunks similar to the original update_index function
    for i in 0..n_new_chunks {
        let chk_idx = start_chunk_idx + i;
        let chk_offset = i * proc_chunk_sz;
        let chk_end_offset = (chk_offset + proc_chunk_sz).min(n_new_docs);
        ...
        // Get bucket cutoffs from the loaded codec
        let b_cutoffs = codec.bucket_cutoffs.as_ref()
            .context("Loaded codec missing bucket_cutoffs")?;
    ...
    // Count total passages across all doclens files
    let doclens_re = regex::Regex::new(r"doclens\.(\d+)\.json")?;
    let mut total_passages = 0;
    for entry in fs::read_dir(idx_path)? {
        let entry = entry?;
    ...
    // First, load the updated IVF data
    ...
    // Load all document lengths including the new ones
    ...
    // Rebuild the codes and residuals tensors with the new data
    let embedding_dim = codec.centroids.size()[1];
