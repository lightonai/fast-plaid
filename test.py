...
def run_evaluation():
    ...
    query_length = {
        "quora": 32, "climate-fever": 64, "nq": 32, "msmarco": 32, "hotpotqa": 32,
        "nfcorpus": 32, "scifact": 48, "trec-covid": 48, "fiqa": 32, "arguana": 64,
        "scidocs": 48, "dbpedia-entity": 32, "webis-touche2020": 32, "fever": 32,
    }
    ...
    model = models.ColBERT(
        ...
        document_length=300,
        device="cpu", # Note: Using CPU as specified in your script
    )
    ...
    index = search.FastPlaid(index=os.path.join("benchmark", dataset_name), device="cpu")
    ...
    scores = index.search(queries_embeddings=queries_embeddings, top_k=20, batch_size=200)
    end_search = time.time()
    search_time = end_search - start_search

    queries_per_second = len(queries) / (end_search - start_search)
    ...
    output_data = {
        "dataset": dataset_name, "indexing": round(indexing_time, 3), "search": round(search_time, 3),
        "qps": round(queries_per_second, 2), "size": len(documents), "queries": num_queries,
